...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Search by name, city, or specialties
    const { data, error, count } = await supabase
      ...
      `, { count: 'exact' })
      .eq('status', 'active')
      .or(`full_name.ilike.%${query}%,display_name.ilike.%${query}%,city.ilike.%${query}%,state.ilike.%${query}%`)
      .order('average_rating', { ascending: false })
      .range(offset, offset + limit - 1);

    if (error) {
      return serverError('Failed to search astrologers', error.message);
    }
    ...
    return ok({ astrologers, total: count || 0, page, limit, query });
  } catch (err) {
    return serverError('Failed to search astrologers', (err as Error).message);
  }
}
